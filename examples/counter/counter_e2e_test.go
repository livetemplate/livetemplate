...
func TestCounterE2E(t *testing.T) {
	...
	t.Run("Increment Counter", func(t *testing.T) {
		var counterText, statusText, conditionalText string
		...
		err := chromedp.Run(ctx,
			...
			chromedp.Click(`button:nth-of-type(1)`, chromedp.ByQuery),
			chromedp.Sleep(200*time.Millisecond),
			chromedp.Click(`button:nth-of-type(1)`, chromedp.ByQuery),
			chromedp.Sleep(200*time.Millisecond),
			chromedp.Click(`button:nth-of-type(1)`, chromedp.ByQuery),
			chromedp.Sleep(500*time.Millisecond), // Wait for WebSocket updates

			// Get updated text
			chromedp.Text(`div > p:nth-of-type(1)`, &counterText, chromedp.ByQuery),
			chromedp.Text(`div > p:nth-of-type(2)`, &statusText, chromedp.ByQuery),
			chromedp.Text(`div > p:last-of-type`, &conditionalText, chromedp.ByQuery),
		)
		...
		if !strings.Contains(counterText, "3") {
			t.Errorf("Counter not incremented correctly, got: %s", counterText)
		}
		if !strings.Contains(statusText, "positive") {
			t.Errorf("Status not updated correctly, got: %s", statusText)
		}
		if !strings.Contains(conditionalText, "Counter is positive") {
			t.Errorf("Conditional text not updated correctly, got: %s", conditionalText)
		}

		t.Log("✅ Increment operations verified")
	})
	...
	t.Run("Decrement Counter", func(t *testing.T) {
		...
		err := chromedp.Run(ctx,
			// Click decrement button 5 times to go negative
			chromedp.Click(`button:nth-of-type(2)`, chromedp.ByQuery),
			...
			// Get updated text
			chromedp.Text(`p:nth-of-type(1)`, &counterText, chromedp.ByQuery),
			chromedp.Text(`p:nth-of-type(2)`, &statusText, chromedp.ByQuery),
			chromedp.Text(`div p:last-of-type`, &conditionalText, chromedp.ByQuery),
		)

		if err != nil {
			t.Fatalf("Failed to decrement counter: %v", err)
		}

		// Should be at -2 now (3 - 5 = -2)
		if !strings.Contains(counterText, "-2") {
			t.Errorf("Counter not decremented correctly, got: %s", counterText)
		}
		if !strings.Contains(statusText, "negative") {
			t.Errorf("Status not updated correctly, got: %s", statusText)
		}
		if !strings.Contains(conditionalText, "Counter is negative") {
			t.Errorf("Conditional text not updated correctly, got: %s", conditionalText)
		}

		t.Log("✅ Decrement operations verified")
	})
	...
	t.Run("Reset Counter", func(t *testing.T) {
		...
		err := chromedp.Run(ctx,
			// Click reset button
			chromedp.Click(`button:nth-of-type(3)`, chromedp.ByQuery),
			chromedp.Sleep(1*time.Second),
		...
		// Should be back at 0
		if !strings.Contains(counterText, "Counter: 0") {
			t.Errorf("Counter not reset correctly, got: %s", counterText)
		}
		if !strings.Contains(statusText, "zero") {
			t.Errorf("Status not updated correctly, got: %s", statusText)
		}
		if !strings.Contains(conditionalText, "Counter is zero") {
			t.Errorf("Conditional text not updated correctly, got: %s", conditionalText)
		}

		t.Log("✅ Reset operation verified")
	})
