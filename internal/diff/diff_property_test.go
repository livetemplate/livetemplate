...
func genDynamics(depth int) *rapid.Generator[map[string]any] {
	if depth <= 0 {
...
func TestCompareTreesAndGetChanges_Property_NoFalseNegatives(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		oldTree := &TreeNode{
			...
			Dynamics: map[string]any{"0": "old_value"},
		}
		...
		newTree := &TreeNode{
			Statics:  []string{"<div>", "</div>"},
			Dynamics: map[string]any{"0": newValue},
		}
...
func buildNestedTree(depth int, leafValue string) *TreeNode {
	if depth <= 1 {
		return &TreeNode{
			...
			Dynamics: map[string]any{"0": leafValue},
		}
	}
