...
// Validates method calls on interface{}-typed struct fields.
func TestStdlibParity_MethodCallOnStructInterfaceField(t *testing.T) {
	type wrapper struct {
		Ctx interface{}
	}

	helper := &testMapHelper{data: map[string]string{"key": "value"}}
	...
	tests := []struct {
		...
		{
			name: "method with arg on interface field",
			tmpl: `<span>{{.Ctx.Get "key"}}</span>`,
			data: wrapper{Ctx: helper},
		},
		{
			name: "conditional on interface field method",
			tmpl: `{{if .Ctx.Has "key"}}found{{else}}empty{{end}}`,
			data: wrapper{Ctx: helper},
		},
		{
			name: "missing key on interface field",
			tmpl: `{{if .Ctx.Has "nope"}}found{{else}}empty{{end}}`,
			data: wrapper{Ctx: helper},
		},
	}
