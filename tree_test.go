	...
	f.Fuzz(func(t *testing.T, journeyJSON string) {
		...
		for i, activity := range journey {
			...
			if i == 0 && activity.Type == "visit" {
				// First render
				tree, err = tmpl.generateInitialTreeWithoutRegistry(todoTemplate, state, todoTemplate)
				if err != nil {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, activity := range tt.journey {
				...
				if i == 0 {
					tree, err = tmpl.generateInitialTreeWithoutRegistry(tt.template, state, tt.template)
				} else {
					if tmpl.lastTree == nil {
						continue
					}
...
<div class="app">
	...
	for i, activity := range journey {
		...
		if i == 0 {
			tree, _ := tmpl.generateInitialTreeWithoutRegistry(template, state, template)
			if err := validator.ValidateUpdate(tree, state, true); err != nil {
