...
**Important**: During JSON marshaling, `RangeData.Statics` (the item template) **overwrites** any node-level `Statics` in the `"s"` key. This means for range nodes on the wire, `"s"` always contains the **item template statics**, not the outer wrapper statics. The server-side TreeNode may have both `Statics` (outer wrapper) and `Range.Statics` (item template), but only the latter appears in the JSON output.
...
Item keys are detected by scanning the range statics for key attributes. The following attributes are recognized (first match in the statics array wins):
...
The position in the statics array where the attribute is found determines which dynamic slot contains the item key. For example, if `data-key="` appears in `statics[0]`, then `dynamics["0"]` is the key.

**Content-based hashing fallback**: If no key attribute is found in the statics, a content-based hash (FNV-1a, 12 hex characters) is generated from the item's dynamic values. This provides stable keys for items without explicit key attributes, but note that any content change will produce a new hash, resulting in remove+insert operations rather than updates. For best performance, use explicit key attributes (e.g., `data-key`).
...
- Each range item MUST have a unique identifier (detected from key attributes in statics, or generated via content-based hashing as fallback)
...
Range operations are set directly at the dynamic position (not wrapped in a `{"d": [...]}` object):
```json
{
  "1": [
