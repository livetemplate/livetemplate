...
- **Audit (start):** no prior phase. Re-verify against live code: `grep '^func \(r \*ConnectionRegistry\) Unregister'` resolves and confirm the `byUser`/`byGroup` cleanup-site shape inside it; no pre-existing `byTopic`/`byTopicPattern` or `segmentMatch` symbol collision in `internal/session/`; the surface is **`UserTopic` only** (confirm no lingering `SessionTopic`/`GlobalTopic` references to mirror); locate the `DispatchRequest` struct (`grep '^type DispatchRequest struct'`) for the optional `Kind` placeholder. Risky assumption to confirm: `Unregister()` is the **sole** connection-cleanup path — specifically that `Connection.Close()` delegates to it rather than running cleanup logic directly, and that no third path bypasses both.
...
- **Audit (start):** read `learnings/phase-0.md` first. Re-verify: the post-action drain site `grep '^func \(h \*liveHandler\) processBroadcasts'`; **both** recursion guards still present (`grep 'calls inside a.*action are ignored'` surfaces both — the *dispatched-action* guard and the *server-initiated-action* guard; `Publish` must guard against both, not just one — the Phase 1 Learnings records which were adapted); the `dispatchBroadcastToGroup` shape (template for `dispatchToTopic`); the `New(...)` validation site where the both-set (`WithTopicACL`+`WithOpenTopics`) hard error must be raised, order-independent; confirm **only `SelfTopic()`** is ACL-exempt (no global carve-out). Risky assumption to confirm: `internal/parse/` does **not** already track client-wired action names — the `V19` symmetry guard needs a new small parse-layer pass; scope its accessor before coding.
...
- **Audit (start):** read `learnings/phase-1.md` through `phase-4.md` and confirm no API drift that would change the removal surface. Re-verify the **exact** in-repo `BroadcastAction` call-site set with the authoritative invocation sweep `grep -rn '\.BroadcastAction(' . --include='*.go'` (call-site invocations only — as of this writing: `e2e/docker/app/main.go`, `broadcast_test.go`, `context_broadcast_test.go`, `lifecycle_integration_test.go`, `navigate_test.go`, plus the `context.go` definition + recursion-guarded internal calls). Plain `grep -rn 'BroadcastAction'` is **not** the call-site sweep — it also returns implementation/comment refs (`mount.go`, `internal/session/registry.go`, `pubsub/types.go`, `session_impl.go`; ~11 files total). `handle_test.go` has **no direct call** but holds `TestEphemeral_BroadcastActionStillWorks`, and `broadcast_test.go` holds `TestBroadcastAction_NoAutomaticPeerDispatch` — both test the removed API and need **repurposing**, not call-site migration; confirm the group-dispatch path's sole caller; confirm the `lvt` 3-file surface **by grepping `../lvt` at Audit time** (as of this writing: `internal/serve/{websocket.go,server.go,websocket_test.go}` — **both** test funcs — but this is a cross-repo claim that may drift; the live grep against `../lvt` is authoritative, not this list); confirm the atomic-removal constraint still holds (no half-removed intermediate build); scope the `V20` removed-API sweep set (the grep that Phase 6 must return zero hits for, across all repos). **State the resolved call-site count precisely in the learnings** — this is a bot-verified factual claim.
...
- [ ] Migrate in-repo `.BroadcastAction(` call sites to `Subscribe(SelfTopic())`+`Publish`+reconciler: `e2e/docker/app/main.go`, `broadcast_test.go`, `context_broadcast_test.go`, `lifecycle_integration_test.go`, `navigate_test.go` (authoritative sweep per the Audit; `handle_test.go` has **no** direct call). Repurpose the tests that exercise the removed API: `TestBroadcastAction_NoAutomaticPeerDispatch` (`broadcast_test.go`) **and** `TestEphemeral_BroadcastActionStillWorks` (`handle_test.go`).
...
- **Audit (start):** read **all** prior `learnings/phase-N.md`. Triage the full Surfaced-Scope & Deferral Ledger (decide ship-in-v1 vs log-as-reconciled-with-Appendix-B in `progress.md` — the proposal's Appendix B stays frozen, the journal entry is the record; there is no `Deferred (post-v1)` section in this spec); re-run the `V15` stale-`Sync` acceptance sweep and the `V20` removed-API sweep (from Phase 5's learnings) to scope the rewrite precisely; confirm the migration targets still exist — the 4 `examples` apps (`landing-demo`/`shared-notepad`/`todos`/`chat`), the 2 `tinkerdown` examples + `sharedAuth` comment, and the `devbox-dash` pin.
...
- **Learnings (exit):** final ledger triage (what shipped vs what was logged as reconciled-with-Appendix-B); `V15`/`V20` status; cross-repo release-tail (docs→examples→pin-bumps) confirmed; the post-wave ledger reconciliation is recorded as the closing journal entry in `progress.md` (Appendix B itself stays frozen).
