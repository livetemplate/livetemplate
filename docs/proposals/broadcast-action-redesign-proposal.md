...
- **Audit (start):** no prior phase. Re-verify against live code: `grep '^func \(r \*ConnectionRegistry\) Unregister'` resolves and confirm the `byUser`/`byGroup` cleanup-site shape inside it; no pre-existing `byTopic`/`byTopicPattern` or `segmentMatch` symbol collision in `internal/session/`; the surface is **`UserTopic` only** (confirm no lingering `SessionTopic`/`GlobalTopic` references to mirror); locate the `DispatchRequest` struct for the optional `Kind` placeholder. Risky assumption to confirm: `Unregister()` is the **sole** connection-cleanup path — specifically that `Connection.Close()` delegates to it rather than running cleanup logic directly, and that no third path bypasses both.
