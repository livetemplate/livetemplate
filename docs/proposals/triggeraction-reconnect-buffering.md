...
- `Session.TriggerAction` (`session_impl.go`) fans an action out to every connection in the session group via `Connection.EnqueueDispatch`. If `registry.GetByGroup(groupID)` returns empty and no broadcaster implementing `pubsub.GroupActionBroadcaster` is configured, it returns `ErrSessionDisconnected`. With a `GroupActionBroadcaster` configured, it returns `nil` even with zero local connections (the action may land on another instance). The plain `pubsub.Broadcaster` interface is *not* sufficient — `TriggerAction` type-asserts to `GroupActionBroadcaster` specifically.
