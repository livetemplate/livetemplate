...
| 2 — Cross-instance (Redis) | not started | — | — | — | `phase-2.md` | ⚠ Inherits a pre-existing flaky `-race` in `pubsub`: `TestReconnect_DoesNotBlockConcurrentPublish` (not Phase 1's — 0 pubsub files changed; flagged on PR #419 cross-repo `-race ./...`, reproduced green locally; analogous to #418's latency flake). Phase 2 owns `pubsub` — fix or skip-guard it + scope Redis `-race` to pubsub/session. See `phase-1.md`. |
