...
4. **Deprecation-log suppression mechanism.** The Phase 3 warning fires when the heuristic resolves *and* `lvt-submitter` was absent. Two options for letting users suppress it:
    ...
    - **Cache mechanics:** the rate-limiting cache (one warning per `(URL path, action name)` tuple per process) should be a **bounded LRU** (e.g., 256 entries) rather than an unbounded `sync.Map`. Apps with dynamic path segments (`/posts/123/edit`, `/posts/456/edit`, ...) would otherwise see one cache entry per unique URL × action for the process lifetime — easy to reach millions of entries on a busy server. The bound is acceptable because if the LRU evicts an old `(path, action)` it just means that one tuple gets re-warned later; the warning is transitional, not strictly once-ever.
    - **Cache key derivation:** prefer the *route template pattern* (e.g., `/posts/{id}/edit`) over the live URL when the parsing layer can access it. If only the live URL is available (likely the case at the message-parsing layer), accept the LRU bound as the safety net.
    - **Concurrency:** whichever store is used, the test-and-store must be atomic. With `sync.Map` use `LoadOrStore`; with a bounded LRU use the library's atomic-add-if-absent (most Go LRU implementations expose this). The two-step `Load` + `Store` pattern has a TOCTOU gap where two concurrent requests both miss the read and both emit the warning.
