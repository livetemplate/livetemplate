...
- **HTTP fetch path (URL-encoded)**: in `livetemplate-client.ts`'s `sendHTTP()` (search anchor), set the analogous `lvt-submitter` form key for URL-encoded form bodies. Verify the function actually serializes URL-encoded forms separately; if it delegates to multipart, this edit is a no-op.
...
3. New e2e test (or extend an existing form submission e2e in the lvt repo at `e2e/livetemplate_core_test.go`) verifying the explicit submitter path round-trips through both the HTTP and WS paths. Adding this is the responsibility of the Phase 2 implementation plan, not the proposal-revision PR.
