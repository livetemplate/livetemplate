...
**`lvt-scroll-away` (client v0.8.33+):** The `lvt-scroll-away="bottom"` attribute toggles a `.visible` CSS class based on the `data-lvt-target` container's distance-to-bottom. The class is added while distance > threshold (user is far from the bottom) and removed once within threshold of the bottom. Threshold is configurable via `--lvt-scroll-threshold` CSS custom property (default: **200px**). This primitive fits "more content below" affordances that fade as the user reaches the end of a list — **not scroll-to-top buttons**. Scroll-to-top requires the inverse semantics (visible when `target.scrollTop > threshold`), which the current implementation doesn't support; tracked at [livetemplate/client#102](https://github.com/livetemplate/client/issues/102). Only `edge="bottom"` is currently supported. Source: `client/dom/scroll-away.ts`.
...
- [x] Consider adding `lvt-scroll-away` scroll-to-top button to Pattern #10 (Infinite Scroll) — **deferred**. Investigation revealed `lvt-scroll-away` only supports `edge="bottom"` (`.visible` when distance to bottom > threshold), which is the wrong primitive for scroll-to-top (needs `scrollTop > threshold`). Filed [livetemplate/client#102](https://github.com/livetemplate/client/issues/102) for a top-edge variant. Patterns.md doc drift around line 2311 (above) is fixed in this same commit.
