...
- **Multi-instance caveat — the error-return pattern silently loops forever under PubSub.** `session_impl.go:108-112` only returns the no-connections error when `!hasRemote`. If a `GroupActionBroadcaster` (e.g., Redis PubSub) is configured, `TriggerAction` returns `nil` even with zero local connections, because the user may be connected to a peer instance. The `err != nil` cancellation pattern as written above will therefore **loop forever** in any multi-instance deployment — and worse, under a persistent PubSub outage it will loop forever while emitting "pubsub publish failed" warnings on every call (see the comment block at `session_impl.go:75-90` for the full lifecycle contract). Patterns that need to work in both single and multi-instance modes MUST use one of:
  - A **hard iteration bound**: `for i := 0; i < 100; i++ { ... }` — the ProgressBar pattern does this implicitly (10 × 500ms = 5s bounded run). Safe for finite work.
  ...
  Session 3's patterns are all bounded by design (LazyLoad: single 2s sleep; ProgressBar: 10-tick bounded loop; AsyncOps: single 2s sleep). No multi-instance unsafety in practice, but document the constraint so future sessions don't copy the `err != nil` pattern into an unbounded loop without realizing it. Do NOT use `context.Context` from `r.Context()` — the framework does not thread a cancellable request context through `Session` (see `livetemplate/livetemplate#303` for the fix that would enable this).
...
- Create a worktree under `.worktrees/<session-name>` in **both repos if the session needs library or client changes**. Session 3 required parallel worktrees (examples + livetemplate) because the Session.TriggerAction wiring gap was discovered mid-session. From the `livetemplate/` repo root, run `grep -n "newLocalSession\|WithSession" mount.go` at the START of any server-push session to verify the framework plumbing exists before writing patterns.
