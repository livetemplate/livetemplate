...
| `internal/keys/loader.go` | **`LoadExistingKeyMappings`** (`loader.go:23`) calls `gen.LoadExistingKeys(node.Range.Items)` directly. In stream mode `Range.Items` is `nil`, so `LoadExistingKeys(nil)` is a Go no-op (range over nil) — the auto-key generator counter stays at 0 and the next render restarts numeric keys at "1", silently re-assigning keys that already exist in `StreamState.Keys`. This breaks key stability for ranges using the default sequential auto-keys (anything without `data-key`/`id` attributes). The fix: when `Range.StreamState != nil && Range.Items == nil`, load the max numeric key from `StreamState.Keys` instead — preserving the same semantics as the items-based path. For `data-key`/hash-based keys this is a no-op (already safe). |
