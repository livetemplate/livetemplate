...
- Adapt the two presence-only helpers: `isComplexInsertionPatternCtx` (replace `_, inOld := ctx.oldByKey[keyStr]` with `_, inOld := ctx.oldKeySet[keyStr]`) and `areAllItemsAtEndCtx` (replace `len(ctx.oldItems) == 0` with `len(ctx.oldKeySet) == 0` AND replace `_, exists := ctx.oldByKey[itemKey]` with `_, exists := ctx.oldKeySet[itemKey]`). `areAllItemsAtStartCtx` needs **no function-body edit** — it iterates only `ctx.addedKeys` and `ctx.newItems`, so its stream-mode behaviour is correct once `ctx.addedKeys` is correctly derived upstream.
