...
**Stream mode applies to any homogeneous range reachable through the static tree; only ranges nested as items inside another range stay on the legacy path.** "Static tree" means: the root, plus any TreeNode reachable by walking through `Dynamics` children — including conditional and `with` branches. `TransitionToStreamMode` recurses through `Dynamics` so a homogeneous range wrapped in `{{if .Cond}}{{range .Items}}…{{end}}{{end}}` or `{{with .Group}}{{range .Items}}…{{end}}{{end}}` still transitions on the second render. The walk does **not** descend into a `Range`'s `Items` — that exclusion is what keeps the wire-format invariant below intact. A range item that itself contains an inner range (e.g. a list of categories where each category renders a list of products) is hashed as a whole at the outer level: when an inner-range item changes, the outer item's full dynamics map (including the nested range's serialized representation) hashes differently → the outer `["u"]` op carries the entire updated inner-range tree as part of the outer item's payload. The inner range's `RangeData` is stored *inside* the outer item's dynamics; it never gets its own retained `lastTree` slot, so it never populates its own `StreamState`. This is consistent with the wire spec — nested-range updates today already serialize the entire inner range when an outer item changes. The §10 `tree_compare.go` row dispatches on the *outer* range's `StreamState` only; `handleRangeMatch` (the nested path) reaches `GenerateRangeStreamOperations` only when the nested range happens to be the outer range of a different `lastTree` slot, which is the recursive case. Net effect: stream mode covers ranges at any static-tree depth; nested ranges in stream-mode items still pay the legacy per-item serialization cost, but only inside the outer item's `["u"]` payload, not as separately retained state. (The Phase 6 `LargeTable` example caught this contract — pre-fix, wrapping the range in `{{if gt (len .Items) 0}}` defeated stream mode silently.)
...
- Extend `rangeContext` with a `fromStream bool` flag and a NEW field `oldKeySet map[string]struct{}` populated on **both** paths (from `oldKeys` on legacy renders, from `StreamState.Keys` on stream renders). The existing `oldByKey map[string]interface{}` is unchanged — it serves two legacy-only value-reading callers (`isPureReorderingCtx`, `generateUpdateOps`) and `DetectPositionField`. See §10's `helpers_range.go` row for the full helper-by-helper adaptation map.
- Adapt the two presence-only helpers: `isComplexInsertionPatternCtx` (replace `_, inOld := ctx.oldByKey[keyStr]` with `_, inOld := ctx.oldKeySet[keyStr]`) and `areAllItemsAtEndCtx` (replace `len(ctx.oldItems) == 0` with `len(ctx.oldKeySet) == 0` (this is the right-hand operand of the compound `if len(ctx.addedKeys) == 0 || len(ctx.oldItems) == 0` guard; leave the `len(ctx.addedKeys) == 0` operand unchanged — it's already stream-mode-safe) AND replace `_, exists := ctx.oldByKey[itemKey]` with `_, exists := ctx.oldKeySet[itemKey]`). `areAllItemsAtStartCtx` needs **no function-body edit** — it iterates only `ctx.addedKeys` and `ctx.newItems`, so its stream-mode behaviour is correct once `ctx.addedKeys` is correctly derived upstream.
...
**Audit checkpoint:** every test in item 1 is exercised by a direct call to `GenerateRangeStreamOperations`, no production code path reaches it yet. Reviewer confirms helper adaptations match the §10 spec (in particular, that `oldKeySet` is non-empty on the stream path — Round 12's regression check).
