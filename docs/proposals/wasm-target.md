...
1. **Parse `main.go` AST** to discover the single `tmpl.Handle(controllerExpr, asStateExpr)` call. Fail with a helpful error if there are zero or more than one (the latter is a multi-page app — see [§7.1](#71-multi-page-apps)). Provide a `--entry=<controllerExpr>:<stateExpr>` flag pair for the multi-Handle case as a v0 escape hatch — e.g., `lvt build wasm --entry='&myapp/app.CounterController{}:livetemplate.AsState(&myapp/app.CounterState{})'`. The two Go expressions (separated by `:`) are emitted verbatim into the generated wrapper. The expressions extracted from AST scan (e.g. `&app.CounterController{}` and `livetemplate.AsState(&app.CounterState{})`) are emitted verbatim into the generated wrapper, so the dev's existing types and constructors are honored.

   **Reserved namespace check.** In the same AST pass, `lvt build wasm` scans for any `mux.Handle(path, ...)` registrations under the `/lvt/` prefix (reserved by the framework for the OPFS service worker — see [§4.5](#45-service-worker-for-opfs)). If found, the build errors with `"path '/lvt/...' conflicts with the framework's reserved namespace; route at a different prefix"`. Devs can't accidentally collide with the SW intercept.

   **Brittle by design (acknowledged):** the AST scan walks the `main()` function body and finds `Handle()` calls *that appear there directly*. Common patterns it does NOT find: a factory like `mux.Handle("/", buildHandler())` where `buildHandler()` calls `tmpl.Handle(...)` internally; a slice of route registrations built dynamically; conditional registration behind an `if`. For these, the error message is actionable: `"no tmpl.Handle(...) call found in main(). If your handler is built in a helper, pass --entry=<controllerExpr>:<stateExpr>."` Devs with helper-built handlers add a single flag; the dev's source stays unchanged.
...
3. **Synthesize an adjacent prerender binary** at the same scratch dir. This is a small server-mode Go program that imports the same package, instantiates the same controller + state, calls `tmpl.Execute(&buf, state)` once, and writes the resulting HTML to stdout. Builds with the default Go toolchain (no `GOOS=js`).
   ...
   **Build-time-safe constraint on Mount.** The prerender invokes the dev's `Mount(zeroState, ctx)` at build time, in a subprocess on the developer machine (or CI runner). Controllers used with `lvt build wasm` MUST be safe to call with zero-value state and a fake initial-mount context — no outbound HTTP, no database connections, no logging to remote services, no filesystem writes outside the scratch dir. Most Mount implementations satisfy this (Mount is typically light initialization). `lvt build wasm` runs the prerender with a 30s timeout and fails the build (non-zero exit + clear error message) if the subprocess hangs, crashes, or exits non-zero. **Apps with Mount work that can't run safely at build time use `lvt build wasm --no-prerender`** (already mentioned in [§3 item 3](#3-at-a-glance)) — coarse but sufficient escape hatch, and it preserves the "no new public APIs" claim in [§2 Design philosophy](#2-design-philosophy) (no new `Context` predicate needed; the build flag controls the prerender phase entirely).
...
**On `data-wasm-size`:** emitted only when `lvt build wasm --dev` is set; useful for developer tooling and the §5 size table; never read by the client runtime. Progress is always derived from `response.headers.get("Content-Length")` (the on-wire byte count); if that's missing, the bar falls back to indeterminate shimmer rather than to a wrong denominator. Production builds omit the attr entirely.
...
The IndexedDB schema: one database named `livetemplate`, one object store named `sessions`, key = `groupID`, value = a small JSON envelope `{"v": <schema-version>, "state": <[]byte JSON from ExtractPersistFields>}`.

**`groupID` is per-deployment, not the fixed `"wasm"` literal.** Two different WASM apps deployed at the same origin (`mysite.com/app1/` and `mysite.com/app2/`) must not corrupt each other's persisted state. Phase 1 derives the `groupID` from a build-time constant injected by `lvt build wasm` — the dev's package import path is a stable, unique-per-build identifier (e.g., `groupID = "myapp/app"` for an app whose `main.go` lives at `myapp/`). Apps deployed to subpaths of the same origin get distinct `groupID`s automatically; same-app deploys at different paths share state (which is usually what the user expects).

**Schema migration.** The envelope's `v` field is the framework's schema version (currently 1). When `WASMSessionStore.restore()` reads a record:
- `v` matches current → unmarshal, hand to `InjectPersistFields`, done.
- `v` is older → run any registered migrations forward, then unmarshal. v0 ships no migrations; if a future framework release changes the envelope structurally, Phase 1+N defines a migration step.
- `v` is newer (user opened an older app version after upgrading and downgrading) → treat as zero-value (rather than risk crash); log a console warning.
- `v` is missing or unparseable → treat as zero-value.

User-data migration (e.g., dev removes a `lvt:"persist"` field, changes its type) is handled by `html/template`'s JSON unmarshaling tolerance — unknown fields are ignored, missing fields stay zero-value. Structural incompatibilities (`int` → `[]int` on the same JSON path) would fail the unmarshal and fall back to zero-value with a console warning. Apps that need explicit field-level migration ship their own migration in their `Mount` (e.g., detect zero-value-after-restore and run a one-time upgrade).
...
In WASM mode the bytes live in OPFS, not on a server. To keep the dev's templates working unchanged, `lvt build wasm` generates a service worker (`dist/lvt-opfs-sw.js`) and the static shell registers it at `scope: "/lvt/"`. The service worker intercepts requests matching `/lvt/opfs-blob/<id>` and serves bytes from OPFS via the standard Fetch API.

The dev's templates reference uploaded files at the framework-reserved `/lvt/opfs-blob/<id>` path:

```html
<img src="/lvt/opfs-blob/{{.AvatarID}}" alt="avatar">
```

Apps with existing server-mode templates at `/files/<id>` (the example in the snippet above this section) keep that route in their server-mode `main.go`; the WASM build's static shell rewrites their templates' `/files/` references to `/lvt/opfs-blob/` automatically as part of the prerender step ([§4.1 step 6](#41-the-lvt-build-wasm-pipeline)). The dev's source stays unchanged across modes.

Alternative considered and rejected: dev plumbs `URL.createObjectURL(blob)` in their templates. This forces the templates to diverge between server mode (where `/files/<id>` is a real HTTP endpoint) and WASM mode (where it's a blob URL). The whole point of this proposal is that the dev's source is unchanged across modes — so the service worker, with its slightly heavier lifecycle, is the right cost to pay. A configurable `--upload-prefix=` flag was also considered and rejected — pointing it at `/` would re-introduce the SW scope-collision risk (see the lifecycle bullet below on why the SW must live at `/lvt/`).

Service worker lifecycle gotchas the proposal owns:

- **Scope is `/lvt/` (not site root).** The SW is registered at `scope: "/lvt/"` and intercepts only `/lvt/opfs-blob/*`. Narrow scope is required so it doesn't collide with an app's existing SW (PWA caching, Workbox, Next.js SW plugin, etc.) — two service workers cannot coexist at the same scope; one would silently displace the other. The `/lvt/` namespace is reserved by the framework; the AST scan in [§4.1 step 1](#41-the-lvt-build-wasm-pipeline) rejects any app route registered under `/lvt/*` with a clear build error.
...
|---|---|---|
| **V — Validation prototype** *(retroactive label for the pre-Phase-0 work; predates this proposal's phase numbering)* | done 2026-05-24 | iPhone smoke pending; sizes pass thresholds; framework compiles to WASM with no refactor needed; `WithRawTemplate` option verified |
...
| **2 — `lvt build wasm` CLI + dev mode** | ~2 weeks | `lvt/commands/build_wasm.go`; AST parse of `main.go`; synthesize `main_wasm.go` + prerender binary; assemble `dist/`; service-worker generation; pre-compression; **`lvt build wasm --dev`** with debounced incremental rebuild + browser live-reload (skips minification/compression for fast iteration — the parity-gap mitigation that [§7.2](#72-hot-reload-in-wasm-dev-mode) calls out; the two-tier-build stretch is still deferred to Phase 2.5) |
...
v0 supports a single `tmpl.Handle(...)` call discovered by AST. Apps with multiple Handle calls (router pattern: `mux.Handle("/", tmpl1.Handle(...))` + `mux.Handle("/admin", tmpl2.Handle(...))`) need a routing story. **The v0 recommended path is per-route bundles** — run `lvt build wasm --entry=...` once per route, deploy each WASM under its own URL prefix. Keeps each blob small (one controller + its templates), scales naturally with the static-CDN deployment model. The alternative — bundle all pages into one WASM blob with client-side `window.location.pathname` selection — is sketched here for completeness but **deferred to Phase 2 design** because it requires multi-Handle AST resolution and shared-state semantics that aren't worked out yet. Apps with many routes would also see the combined blob grow substantially (several controllers + all template strings baked in), which is the wrong default.
