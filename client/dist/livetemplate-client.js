var LiveTemplateClient = (() => {
  ...
  var LiveTemplateClient = class {
    ...
    loadPersistentCache() {
      try {
        const cacheData2 = localStorage.getItem("livetemplate-cache");
        if (cacheData2) {
          const parsed = JSON.parse(cacheData2);
          if (parsed.statics) {
    ...
    savePersistentCache() {
      try {
        ...
        localStorage.setItem("livetemplate-cache", JSON.stringify(cacheData2));
      } catch (error) {
    ...
    applyDiffUpdate(fragment) {
      ...
      if (!element) {
        console.warn(`Element with lvt-id="${fragment.id}" or id="${fragment.id}" not found`);
        return;
      }
    ...
    applyContentUpdate(element, newContent, fragmentId) {
      ...
      if (voidElements.includes(element.tagName)) {
        ...
        if (attrMatch) {
          ...
          const tempElement2 = tempDiv.firstChild;
          if (tempElement2) {
            Array.from(tempElement2.attributes).forEach((attr) => {
      ...
      const doc2 = parser.parseFromString(newContent, "text/html");
      let tempElement = doc2.body.firstChild;
      if (!tempElement) {
        tempElement = doc2.head.firstChild;
      }
      if (!tempElement) {
      ...
      morphdom_esm_default(element, tempElement, {
        ...
        childrenOnly: false
        // FIXED: Allow element attributes to be updated too
      });
