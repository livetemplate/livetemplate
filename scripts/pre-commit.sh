...
if [ -z "$LINT_EXIT_CODE" ] || [ "$LINT_EXIT_CODE" -eq 0 ]; then
    ...
    echo ""
    echo "    For (b) — inline the constant at the function body. If"
    ...
    echo ""
    echo "    For (c) — drop the return value and update callers. If the"
    echo "    work behind the return is intentional (e.g., timing in a"
    echo "    benchmark), keep the work but discard the value with '_ ='"
    echo "    and document why."
    echo ""
