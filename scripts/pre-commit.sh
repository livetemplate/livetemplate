...
if [ -z "$LINT_EXIT_CODE" ] || [ "$LINT_EXIT_CODE" -eq 0 ]; then
    ...
    echo "    a defensive sentinel-error return:"
    echo "      var errNilMsg = errors.New(\"handler got nil message\")"
    echo "      handler := func(msg *T) error {"
    echo "          if msg == nil { return errNilMsg }"
    echo "          ..."
    echo "      }"
    echo "    This doubles as a contract validator and is preferable to"
    echo "    '_ = msg' suppression."
    echo ""
